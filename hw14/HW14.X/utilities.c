...
{
    // Select 14 pins for motor direction control
    TRISBCLR = 0b100000000000000; // 14
    MOTOR_1_DIRECTION = 0; // Clockwise

//    // DC motor control -> 20kHz Timer for PWM for each motor
...
//    OC1CONbits.OCTSEL = 1;        // Use Timer 3
    ...
    // Servo control -> 50Hz Timer for PWM for each motor
    T3CONbits.TCKPS = 0b111;      // Timer3 prescaler
    PR3 = PR3_PERIOD - 1;         // Period
    TMR3 = 0;                     // initial TMR3 count is 0
    T3CONbits.ON = 1;             // turn on Timer3
    // Assign OC1 to B15 -> Motor 1 PWM
