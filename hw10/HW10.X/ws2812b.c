...
void ws2812b_setup() {
    ...
    // initialize output pin RB10 as off
    TRISBCLR = 0b10000000000; // Make TRIS RB10 an output    
    LATBbits.LATB10 = 1;      // Initialize it off
}
...
void ws2812b_setColor(wsColor * c, int numLEDs) {
    ...
    volatile unsigned int delay_times[2*3*8 * numLEDs];
    ...
    for (i = 0; i < numLEDs; i++) {
        ...
        for (j = 7; j >= 0; j--) {
            // RED - if the bit is a 1
            if ((c[i].r >> j) & 1) { /* identify the bit in c[i].r, is it 1 */
                // the high is longer
        ...
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if ((c[i].g >> j) & 1) { /* identify the bit in c[].g, is it 1 */
                // the high is longer
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME;
                nB++;
                delay_times[nB] = delay_times[nB - 1] + LOWTIME;
        ...
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if ((c[i].b >> j) & 1) {  /* identify the bit in c[].b, is it 1 */
                // the high is longer
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME;
                nB++;
                delay_times[nB] = delay_times[nB - 1] + LOWTIME;
